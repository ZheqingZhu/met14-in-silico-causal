...
def set_global_determinism(seed):
    """
    Lock the global pseudo-random number generators (PRNG) to ensure
    strict computational reproducibility across different runs.
    """
...
def run_full_e_step(data_dir, n_patients=3000, current_seed=2026):
    """
    Execute the full Copula E-step engine to construct the synthetic cohort.
    """
    print("\n" + "=" * 60)
    print(f"🤖 Initiating Copula E-step Engine | Current PRNG Seed: {current_seed}")
    print("=" * 60 + "\n")

    # [Core Module]: Enforce strict determinism for the current execution
    ...
    # Step 2: Construct the synthetic cohort
    print("\n------------------------------------------------------------")
    ...
    # [Data Output]: Append seed to the filename to prevent overwriting specific cohorts
